(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new Swiper(".cases__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: "auto",
            ...
            speed: 800,
            navigation: {
                prevEl: ".navigation-cases__prev",
                nextEl: ".navigation-cases__next"
            },
            breakpoints: {
                320: {
                    spaceBetween: 20
    ...
    var Dimensions = class {
        ...
        constructor(wrapper, content, {autoResize = true, debounce: debounceValue = 250} = {}) {
            ...
            if (autoResize) {
                ...
                if (this.wrapper instanceof Window) window.addEventListener("resize", this.debouncedResize); else {
    ...
    var Lenis = class {
        ...
        checkOverflow() {
            if ([ "hidden", "clip" ].includes(this.overflow)) this.internalStop(); else this.internalStart();
        }
        ...
        onClick=event => {
            ...
            if (this.options.stopInertiaOnNavigate) if (linkElementsUrls.some(targetUrl => currentUrl.host === targetUrl.host && currentUrl.pathname !== targetUrl.pathname)) {
                this.reset();
                return;
            }
        };
        ...
        scrollTo(_target, {offset = 0, immediate = false, lock = false, programmatic = true, lerp = programmatic ? this.options.lerp : void 0, duration = programmatic ? this.options.duration : void 0, easing = programmatic ? this.options.easing : void 0, onStart, onComplete, force = false, userData} = {}) {
            ...
            if (typeof target === "string" && [ "top", "left", "start", "#" ].includes(target)) target = 0; else if (typeof target === "string" && [ "bottom", "right", "end" ].includes(target)) target = this.limit; else {
                ...
                if (typeof target === "string") {
                    ...
                    if (!node) if (target === "#top") target = 0; else console.warn("Lenis: Target not found", target);
    ...
    window.addEventListener("scroll", () => {
        header.classList.toggle("_scrolled", window.scrollY > 0);
    });
    const cards = document.querySelectorAll(".services__card");
    const obs = new IntersectionObserver(entries => {
        entries.forEach(e => {
            if (e.isIntersecting) {
                cards.forEach(c => c.classList.remove("_active"));
    ...
    document.querySelectorAll(".select").forEach(select => {
        ...
        const input = select.querySelector(".select__value");
        ...
        options.forEach(option => {
            option.addEventListener("click", () => {
                ...
                placeholder.classList.add("_selected");
                input.value = option.textContent;
                options.forEach(o => o.classList.remove("_active"));
    ...
    if (stepSection && stepItems.length && window.innerWidth > 991) {
        ...
        function applyProgress() {
            const t = totalProgress / COUNT;
            stepItems.forEach((item, i) => {
                const speed = 1 + (COUNT - 1 - i) * SPREAD;
                const p = Math.max(0, Math.min(1, t * speed));
                const maxPadding = (i + 1) * 180;
                item.style.paddingTop = `${(1 - p) * maxPadding}px`;
        ...
        window.addEventListener("wheel", e => {
            ...
            if (goingDown && targetProgress >= COUNT) {
                unlock();
                return;
            }
            ...
            e.preventDefault();
            lock();
            if (goingDown) targetProgress = Math.min(COUNT, targetProgress + STEP); else targetProgress = Math.max(0, targetProgress - STEP);
            if (!rafId) rafId = requestAnimationFrame(animate);
        }, {
            passive: false
        });
    }
    document.querySelectorAll(".partners__track-wrap").forEach(wrapper => {
        const list = wrapper.querySelector(".partners__list");
        if (!list) return;
        const originalItems = Array.from(list.children).map(item => item.cloneNode(true));
        const direction = wrapper.dataset.direction || "left";
        const speed = Number(wrapper.dataset.speed) || 70;
        let animationFrameId = null;
        let resizeTimeout = null;
        let buildId = 0;
        function createGroup(isClone = false) {
            ...
            originalItems.forEach(originalItem => {
                ...
                if (isClone) item.querySelectorAll("img").forEach(image => {
                    image.alt = "";
                });
        ...
        async function buildMarquee() {
            ...
            list.appendChild(firstGroup);
            await waitForImages(firstGroup);
            ...
            const groupsCount = Math.max(3, Math.ceil(wrapperWidth / groupWidth) + 2);
            for (let index = 1; index < groupsCount; index++) list.appendChild(createGroup(true));
            let position = direction === "right" ? -groupWidth : 0;
            let previousTime = performance.now();
            list.style.transform = `translate3d(${position}px, 0, 0)`;
            list.classList.add("is-ready");
            function animate(currentTime) {
                const deltaTime = Math.min((currentTime - previousTime) / 1e3, .05);
    ...
    if (scrollUpButton) {
        const showButtonOffset = 500;
        const toggleScrollUpButton = () => {
            scrollUpButton.classList.toggle("_show", window.scrollY > showButtonOffset);
        };
